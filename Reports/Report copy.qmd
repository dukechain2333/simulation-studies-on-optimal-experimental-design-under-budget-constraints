...
normal_alpha_results_optimal_table <- kable(design_performance_normal_alpha %>%
  group_by(alpha) %>%
  summarize(mean_mse = mean(mse), G = G[which.min(mse)], R = R[which.min(mse)], total_cost = total_cost[which.min(mse)]), caption = "")
...
tbl_summary(design_performance_poisson_gamma2 %>% select(c(bias, mse, coverage, gamma2)), by = gamma2) %>%
  add_p()
...
tbl_summary(design_performance_normal_sigma2 %>% select(c(bias, mse, coverage, sigma2)), by = sigma2) %>%
  add_p()
...
```{r poisson_sigma2, eval=FALSE, include=FALSE}
budget <- 10000
c1 <- 50
c2 <- 10
alpha <- 5
...
tbl_summary(design_performance_poisson_sigma2 %>% select(c(bias, mse, coverage, sigma2)), by = sigma2) %>%
  add_p()
